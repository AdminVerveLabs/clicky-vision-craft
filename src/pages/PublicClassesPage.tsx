...
const PublicClassesPage = () => {
  ...
  return (
    <div>
      <section className="pt-[100px] md:pt-[120px] pb-12 md:pb-16 bg-white">
      ...
      {/* Details Bar */}
      <section className="py-16 md:py-20 px-6 bg-gray-light">
        <div className="max-w-[1000px] mx-auto">
          <h2 className="font-serif text-[26px] md:text-[32px] font-extrabold text-dark text-center mb-10">What every class includes</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <div key={item.title} className="text-center">
                <div className="w-14 md:w-16 h-14 md:h-16 rounded-full bg-white flex items-center justify-center mx-auto mb-3 text-2xl md:text-3xl shadow-[0_2px_8px_rgba(0,0,0,0.05)]">{item.icon}</div>
                <h4 className="font-sans text-[14px] md:text-[15px] font-bold text-dark mb-1">{item.title}</h4>
                <p className="font-sans text-[12px] md:text-[13px] text-gray">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
