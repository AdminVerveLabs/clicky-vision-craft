...
const CTAButton = ({ children, variant = "primary", size = "md", onClick }: CTAButtonProps) => {
  ...
  const base = `inline-flex items-center gap-2 font-semibold cursor-pointer font-sans transition-all duration-200 uppercase tracking-widest ${isStamp ? "rounded-tl-2xl rounded-tr-lg rounded-bl-lg rounded-br-2xl hover:-translate-y-1.5" : "rounded-full hover:-translate-y-0.5 hover:opacity-90"}`;
