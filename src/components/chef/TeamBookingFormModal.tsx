...
interface TeamBookingFormModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
const occasionOptions = [
  "Team Building Events", "All-Hands & Townhalls", "Onboarding & Culture",
  "Client Entertainment", "Holiday & Celebrations", "Custom Experiences",
];

const locationOptions = ["Our Kitchen", "Your Office", "Your Venue", "Virtual"];
const packageOptions = ["Essentials – $50/pp", "Classic – $95/pp", "Premium – $150/pp", "VIP – $225/pp"];
const addOnOptions = ["Event Videography ($350)", "Extended Catering ($25/pp)", "Custom Aprons & Swag ($18/pp)"];
...
const TeamBookingFormModal = ({ isOpen, onClose }: TeamBookingFormModalProps) => {
  ...
  const submit = () => {
    toast.success("We'll be in touch within 24 hours!", {
      description: "Thanks for your inquiry — Chef Joey is excited to work with your team.",
    });
    setForm(initialForm);
    setStep(0);
    onClose();
      ...
      <div className="relative bg-white rounded-3xl max-w-[900px] w-full max-h-[90vh] overflow-hidden shadow-2xl flex">
        ...
        <div className="flex-1 flex flex-col overflow-y-auto">
          ...
          <div className="p-8 md:p-10 flex-1">
            <h2 className="font-serif text-[26px] font-extrabold text-dark mb-1">Book a Chat with Chef Joey</h2>
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    <Input type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="you@company.com" className={`rounded-lg ${errors.email ? "border-destructive" : ""}`} />
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="font-sans text-sm font-medium text-dark mb-1.5 block">Company</label>
                    <Input value={form.company} onChange={(e) => set("company", e.target.value)} placeholder="Company name" className="rounded-lg" />
                  </div>
                  <div>
            ...
            {step === 1 && (
              <div className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  <div>
                    <label className="font-sans text-sm font-medium text-dark mb-1.5 block">Estimated Group Size *</label>
                    <Input type="number" value={form.groupSize} onChange={(e) => set("groupSize", e.target.value)} placeholder="e.g. 20" className={`rounded-lg ${errors.groupSize ? "border-destructive" : ""}`} />
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                  ...
                  <div>
                    <label className="font-sans text-sm font-medium text-dark mb-1.5 block">Location Preference *</label>
...
export default TeamBookingFormModal;
